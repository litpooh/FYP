    ...
    //                        }
            ...
            if showingResult {
                GeometryReader{ proxy in
                    ...
                    CheckResult(show: $showingResult, photo: $image)
...
struct CheckResult: View {
    ...
    @State var result_category:Category = Category(name: "", collection_image: "loading", recyclables: [])
            ...
            VStack {
                ...
                VStack(spacing: 25){
                    ...
                    if is_contaminated == true {
                        Text("The item seems to be contaminated! Please clean the item before recycling, or throw it away if there is still dirt left on. ")
                            .font(.body)
                            .fontWeight(.semibold)
...
//            )
        ...
        .onAppear{
            ...
            CoreMLClassifier().classify(image: photo!){ result_recyclable in
                ...
                print("Coreml result recyclable: "+result_recyclable)
                print("Coreml result category: "+self.result_category.name)
                
                if (photo != nil) && (self.result_category.name != "") && (self.result_category.name != "loading") {
                ...
                if (self.result_category.name == "" || self.result_category.name == "loading") {
                    print("As the confidence of coreml is less than 70%")
                    print("Pass to Image labelling for double checking")
                        ...
                        imagelabeller.runRequestOnBackgroundThread(request) { result in
                            ...
                            categorize(label_results: label_results_returned) { return_category in
                                ...
                                if (self.result_category.name == "" || self.result_category.name == "loading") {
                                    ...
                                    print("Both coreml and image labelling believe it is Non Recyclable")
                                }
