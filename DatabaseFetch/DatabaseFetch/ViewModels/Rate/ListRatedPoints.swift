...
struct ListRatedPoints: View {
    ...
    var body: some View {
        ...
        .navigationTitle("Rated Points")
        .onAppear{
            if filterStatus.filtered == false {
                Database().listRatedPoints(userLocation2D: userlocation, email: email) { points in
...
struct ListRatedPoints_Previews: PreviewProvider {
